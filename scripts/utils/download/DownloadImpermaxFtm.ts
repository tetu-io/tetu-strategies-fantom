...
async function collect(pool: string, signer: SignerWithAddress) {
  ...
  if (underlying === Misc.ZERO_ADDRESS) {
    return '';
  }
  ...
  let price = 0;
  try {
    price = +utils.formatUnits(await calculator.getPriceWithDefaultOutput(underlying));
  } catch (e) {
    console.error('can not fetch price for ' + underlyingName);
  }
